            ...
            <div className="cart--dropdown">
                  ...
                  return (
                    <div key={key} className="cart__item--container">
                      <div className="cart__item--info">
                        <span>{item.name}</span>
                        <span>
                          {(item.prices[this.props.currency]?.amount).toFixed(
                            2
                          )}
                        ...
                        <div className="attributes__container">
                          {item?.selectedAttr?.attributes.map((attr, key) => {
                            ...
                            return item.attributes.length ? (
                              ...
                              >
                                {attr.type !== "swatch"
                                  ? item.attributes[key]?.items[attrID[key]]
                                      ?.value
                                  : ""}
                              </button>
                            ) : (
                      ...
                      <div style={{ display: "flex" }}>
                        <div className="cart__item--counter">
                          ...
                          <span>{item.qty}</span>
                          <button onClick={() => subOne(item.id, item.qty)}>
                            -
                          </button>
                        </div>
                        <div>
              ...
              <div className="cart__total--amount">
                ...
                <span>
                  {totalSum.toFixed(2)}
                  {
                    this.props.cartProducts[0]?.prices[this.props.currency]
                      ?.currency.symbol
                  }
                </span>
              </div>
                ...
                <button
                  ...
                  onClick={() => {
                    this.props.cartProducts.length
                      ? alert(
                          `checked out ${totalSum.toFixed(2)}${
                            this.props.cartProducts[0]?.prices[
                              this.props.currency
                            ]?.currency.symbol
                          }`
                        )
